...
class V_VOID {} // placeholder — removed below
...
class InspectionRepository {
  ...
  Future<void> saveAnswer(AnswerModel answer) async {
    final db = await AppDatabase.instance.database;
    final existing = await db.query('inspection_answers',
        where: 'inspection_id = ? AND question_id = ?',
        whereArgs: [answer.inspectionId, answer.questionId], limit: 1);

    if (existing.isNotEmpty) {
      final id = existing.first['id'] as int;
      await db.update('inspection_answers', {...answer.toMap(), 'id': id},
  ...
  Future<Map<String, AnswerModel>> getAnswersForInspection(int inspectionId) async {
    final db = await AppDatabase.instance.database;
    final rows = await db.query('inspection_answers',
